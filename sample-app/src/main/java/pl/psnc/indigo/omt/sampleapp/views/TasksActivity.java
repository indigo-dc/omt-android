        ...
        service.performTokenRequest(resp.createTokenExchangeRequest(),
            new AuthorizationService.TokenResponseCallback() {
              @Override public void onTokenRequestCompleted(TokenResponse response,
                ...
                if (response != null) {
                  // exchange succeeded
                  AuthState authStateAfter = IAMHelper.readAuthState(getApplicationContext());
  ...
  public void getTasks(final Comparator<Task> comparator) {
    ...
    Indigo.getTasks(TaskStatus.ANY, authState, new TasksCallback() {
      ...
      @Override public void onError(Exception e) {
        Log.e(TAG, (e.getMessage() != null) ? e.getMessage() : "Something wrong happend here!");
      }
    });
  }
          ...
          try {
            ...
            if (!tasks.isEmpty()) {
              if (mHandler.get() != null) {
                mHandler.get()
                    .postDelayed(new PeriodicRunnable(mList, mAdapter.get(), mHandler.get()),
                        REFRESH_INTERVAL);

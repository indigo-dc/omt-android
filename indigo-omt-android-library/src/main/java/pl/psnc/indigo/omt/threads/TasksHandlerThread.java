...
public class TasksHandlerThread extends ApiHandlerThread implements ApiCallWorkflow {
  ...
  public TasksHandlerThread(String status, String application, Handler workerHandler,
  ...
  @Override public void networkWork(String token) {
    ...
    try {
      if (mApplication != null && mStatus != null) {
        final TasksWrapper taskWrapper = mTaskAPI.getTasks(mStatus, mApplication);
            ...
            @Override public void run() {
              if (taskWrapper != null && taskWrapper.getTasks() != null) {
                ...
                if (mCallback.get() != null) {
                  ...
                  ((TasksCallback) mCallback.get()).onSuccess(taskWrapper.getTasks());
                }
              }
        ...
        final TasksWrapper taskWrapper = mTaskAPI.getTasks(mStatus);
        if (mResponseHandler.get() != null) {
          mResponseHandler.get().post(new Runnable() {
            @Override public void run() {
              if (taskWrapper != null && taskWrapper.getTasks() != null) {
        ...
        final TasksWrapper taskWrapper = mTaskAPI.getTasks();
        if (mResponseHandler.get() != null) {
          mResponseHandler.get().post(new Runnable() {
            @Override public void run() {
              if (taskWrapper != null && taskWrapper.getTasks() != null) {
